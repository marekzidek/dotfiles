...
"This is the function passing last visualMark to the alternate visualMark
function! s:AlternateVisualMark() "{{{
    ...
    "TODO: here we should get the mark from last set one, if exists
    " If so, the mark should be obtained through s:GetVisualMark and saved
    " into a variable and passed to an alternate register
    ...
    let alternateMark = "k"
    " fill the alternate mark 'k' with the old one
    echo coordinates
    let g:visualMarks[entry][alternateMark] = coordinates
    " and save it to the file. But I am sure we don't need to do this each time.
    call s:SaveVariable(g:visualMarks, g:filen)
...
"This is the function for saving alternate the alternate visualMark
function! s:AlternateVisualMarkSave(...) "{{{
    ...
    let noSuchMark = 1
    if has_key(g:visualMarks, entry)
        if has_key(g:visualMarks[entry], alternateMark)
            let noSuchMark = 0
        endif
    endif
    ...
    if noSuchMark
        echom "no Such mark " . alternateMark . " for this buffer."
    else
        " Then we can safely get back to this selection!
        let coordinates = g:visualMarks[entry][alternateMark]
    endif
    ...
    " fill the alternate mark 'k' with the old one
    echo coordinates
    let g:visualMarks[entry][mark] = coordinates
    " and save it to the file. But I am sure we don't need to do this each time.
    call s:SaveVariable(g:visualMarks, g:filen)
...
vnoremap <unique> <script> <Plug>VisualMarksAlternateVisualMark <SID>AlternateVisualMark
nnoremap <unique> <script> <Plug>VisualMarksAlternateVisualMarkSave <SID>AlternateVisualMarkSave
...
vnoremap <SID>AlternateVisualMark    <esc>:call <SID>AlternateVisualMark()<CR>
nnoremap <SID>AlternateVisualMarkSave      :call <SID>AlternateVisualMarkSave()<CR>
